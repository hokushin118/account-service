...
def get_request_body_or_none() -> Union[Any, None]:
    """
    Retrieves the request body from a Flask request, handling both JSON and non-JSON data.

    Returns:
        The request body (dict for JSON, str for other types), or None if the request body is empty.
    """
    if not request.data:
        return None
    ...
    if request.is_json:
        try:
            ...
            return None
        except Exception as gen_err:  # pylint: disable=W0703
            logger.error(
...
def on_send_error(kafka_err: Exception) -> None:
    """Callback function for a failed Kafka message send.
     ...
     """
    logger.error("Error occurred during Kafka send: %s", kafka_err)
...
def audit_log_kafka(
    ...
    def wrapper(*args, **kwargs) -> Response:
        ...
        try:
            ...
            # Get request body.
            request_body = get_request_body_or_none()
            ...
            producer.send(
                ...
                lambda metadata, key=key_value: on_send_success(
                    metadata,
                    key
                )
            ).add_errback(
        ...
        except Exception as audit_err:  # pylint: disable=W0703
            logger.error(
